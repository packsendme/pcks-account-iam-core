...
public class UserService {
	...
	public ResponseEntity<?> getUsernameAccessCheck(String username, String usernamenew) {
		...
		try{
			...
			if(userFind == null) {
				...
				Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.USERNAME_REGISTER, HttpExceptionPackSend.USERNAME_REGISTER.toString(), userFind);
				return new ResponseEntity<>(responseObj, HttpStatus.ACCEPTED);
			}
			// User already exists in the database 
			else {
				Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.FOUND_USER, HttpExceptionPackSend.FOUND_USER.toString(), null);
	...
	public ResponseEntity<?> allowUserAccessFirstRegister(String username, String password) {
		...
		try {
			...
			Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.USER_ACCESS_CREATED, HttpExceptionPackSend.USER_ACCESS_CREATED.toString(), null);
			return new ResponseEntity<>(responseObj, HttpStatus.ACCEPTED);
		}
		catch (Exception e) {
			Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.FAIL_EXECUTION, HttpExceptionPackSend.FAIL_EXECUTION.toString(), null);
			return new ResponseEntity<>(responseObj, HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	public ResponseEntity<?> cancelUserAccess(String username) {
		try {
			...
			Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.USER_DELETE, HttpExceptionPackSend.USER_DELETE.toString(), null);
			return new ResponseEntity<>(responseObj, HttpStatus.ACCEPTED);
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	public ResponseEntity<?> getSMSValidateProcess(String username, String usernameNew, String smscode) {
		try {
			if(getSMSCodeCheck(username,smscode) == MicroservicesConstants.SMS_VALIDATE_SUCCESS) {
				...
				if(usernameNew != null) {
					...
					if(opResultAccount.getStatusCode() == HttpStatus.ACCEPTED) {
						...
						if(opResultUser.getStatusCode()  == HttpStatus.ACCEPTED) {
							Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.UPDATE_ACCOUNT, HttpExceptionPackSend.UPDATE_ACCOUNT.toString(), null);
				...
				Response<UserModel> responseObj = new Response<UserModel>(HttpExceptionPackSend.NOT_FOUND_SMS_CODE, HttpExceptionPackSend.NOT_FOUND_SMS_CODE.toString(), null);
